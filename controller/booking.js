...
const dateAndTimeChecking = (time, date) => {
    return new Promise(resolve => {
        dbConnection.con.query("SELECT waktuService, tanggalService FROM bookingList WHERE tanggalService = ? AND waktuService= ?; ", [time, date], (err, rows, fields) => {
...
const priceChecking1 = (merkMobil, tipeMobil, jenisPerawatan, detailPerawatan) => {
    return new Promise(resolve => {
        dbConnection.con.query("SELECT harga FROM carTreatment WHERE merkMobil = ? AND tipeMobil= ? AND jenisPerawatan= ? AND detailPerawatan= ?;", [merkMobil, tipeMobil, jenisPerawatan, detailPerawatan], (err, rows, fields) => {
            err ? console.log(err) : resolve(rows);
        })
    })
}
...
exports.showBookingDetail = (req, res) => {
...
exports.showEditBooking = (req, res) => {
    res.render('bookingDetails', {
...
exports.postPriceChecking = async (req, res) => {
    // Set Global Variable
    let bookingData = {
        namaPemilik: req.body.namaPemilik,
    ...
    let priceTotal;
    let rawTotal1, rawTotal2, rawTotal3;
    let priceTotal1, priceTotal2, priceTotal3;
    ...
    if (clientCar.merkMobil.length > 1 || clientCar.tipeMobil.length > 1 || clientCar.jenisPerawatan.length > 1 || clientCar.detailPerawatan.length > 1) {
        ...
        let error_msg = "Silahkan mengisi waktu booking servis anda."
        ...
        let error_msg = "Silahkan mengisi data mobil anda."
        req.flash('error', error_msg)
        res.redirect('/editBooking')
        ...
        if (log.length > 0) {
            let error_msg = "Tanggal dan waktu bookingan anda tidak tersedia. Silahkan untuk menginput kembali."
            req.flash('error', error_msg)
            res.redirect('/editBooking')
...
exports.postBook = (req, res) => {
    ...
    if (!errors) {
        ...
        req.getConnection(function (err, con) {
            dbConnection.con.query('INSERT INTO bookingList SET ?', clientData, function (err, result) {
                if (err) {
                    req.flash('error', err)
